...
export const NewsGrid: React.FC<NewsGridProps> = ({ news, loading, error }) => {
    ...
    const getLocalizedContent = (item: NewsItem) => {
        return {
            title: (isKhmer && item.title_km) ? item.title_km : item.title_vi,
            excerpt: (isKhmer && item.excerpt_km) ? item.excerpt_km : item.excerpt_vi
        };
    };
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-12">
                        <h2 className="text-3xl md:text-4xl font-serif font-bold text-[#6B2C2C] mb-4">
                            {t('news.latest_news', 'TIN TỨC HỌC VIỆN')}
                        </h2>
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="text-center mb-14">
                    ...
                    <p className="text-[#8B4513] max-w-2xl mx-auto">
                        {t('news.latest_subtitle', 'Cập nhật những hoạt động Phật sự, văn hóa và lễ hội mới nhất từ Học viện')}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {news.map((item, idx) => {
                        const { title, excerpt } = getLocalizedContent(item);
                        return (
                            <article
                                key={item.id}
                            ...
                            >
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex items-center gap-2 text-[#8B4513] text-sm mb-3">
                                        ...
                                        <span>{item.published_at ? new Date(item.published_at).toLocaleDateString(isKhmer ? 'km-KH' : 'vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : (isKhmer ? 'ទើប​ធ្វើ​បច្ចុប្បន្នភាព' : 'Mới cập nhật')}</span>
                                    </div>
                                    ...
                                    <p className="text-sm text-[#8B4513]/80 mb-5 line-clamp-2 leading-relaxed">
                                        {excerpt || (isKhmer ? 'មើលព័ត៌មានលម្អិត...' : 'Xem chi tiết bài viết...')}
                                    </p>
                                    ...
                                    >
                                        <span>{t('common.read_more', 'Đọc thêm')}</span>
                ...
                <div className="text-center mt-14">
                    ...
                    >
                        <span>{t('news.view_all', 'Xem tất cả tin tức')}</span>
                        <ArrowRight size={20} />
                    </Link>
                </div>
            </div>
